...
# Create knee plots. The position of the "knee" will inform how many cells to 
#include in the initial expression matrix

pdf('KneePlots.pdf')
lapply(mice, function(i) kneeplot(i, '/pine/scr/s/e/seth96/', 10000))
dev.off()


#Repeat function for GSMO
mice1 <- paste0("G", c(6:11))
kneeplot <- function(mouse, root, xlim){
  message('Working on Mouse: ', mouse)
  rh <- read.table(paste0(root,
                          mouse,
...
# Create knee plots. The position of the "knee" will inform how many 
#cells to include in the initial expression matrix

pdf('KneePlots.pdf')
lapply(mice1, function(i) kneeplot(i, '/pine/scr/s/e/seth96/', 10000))
dev.off()

#Importing the data matrices
##Source local libaries
...
# Establish base variables for importing cells.
#This is to lighten the Seurat object if any 
#very low-read cells were included below the knee.

min.genes <- 10
min.cells <- 0

#function used to generate Seurat object

prep.Seurat <- function(i){
  message('Starting with Sample ', i)
  df <- fread(paste0('/pine/scr/s/e/seth96/MSMO/', i, '/out/dge.txt'),
              stringsAsFactors = F,
              data.table = F, sep = "\t", header = T)
  rownames(df) <- df[, "GENE"]
  df[, "GENE"]<- NULL
  colnames(df) <- paste(as.character(i), colnames(df), sep="_")
  dat <- CreateSeuratObject(counts = df, min.cells = min.cells,
                            min.features = min.genes, names.delim = "_", names.field = 1)
  dat[["Replicate"]] <- Idents(object = dat)
  return(dat)
}
...
MSMO <- merge(seurats[[1]], seurats[2:5])

#repeat the process for GSMO

mice <- paste0("G", c(6:11))
seurats <- list()
min.genes <- 10
min.cells <- 0
prep.Seurat <- function(i){
  message('Starting with Sample ', i)
  df <- fread(paste0('/pine/scr/s/e/seth96/GSMO/', i, '/out/dge.txt'),
              stringsAsFactors = F,
              data.table = F, sep = "\t", header = T)
  rownames(df) <- df[, "GENE"]
  df[, "GENE"]<- NULL
...
seurats <- lapply(mice, function(x) prep.Seurat(x))
GSMO <- merge(seurats[[1]], seurats[2:6])
```
...
write.xlsx(out, file = paste0('GM_SEED_cluster_',i,'_markers.xlsx'))
}
...
pdf("figure_2d.pdf")
DimPlot(gsmo, group.by=c("stromal"), order = c("Meg3","Sox10","C1qb",
"Aqp4","Col3a1","Pecam1","Rsph1","Mog",'zzz'),
cols = rev(c(colors.use[14:19],colors.use[21:22],"lightgrey")),pt.size = 1)

DimPlot(msmo, group.by=c("stromal"), order = c("Meg3","Sox10","C1qb",
"Aqp4","Col3a1","Pecam1","Rsph1","Mog",'zzz'),
cols = rev(c(colors.use[14:19],colors.use[21:22],"lightgrey")),pt.size = 1)
dev.off()
...
#make gene list for cluster 7 comparing GSMO cluster 7
...
gsmo1 <- subset(x = gsmo, ident = 7)
msmo1 <- subset(x = msmo, ident = c(0:6,8:13))
...
clust0$ratio <- clust0[,3]/(clust0[,4]+.0000001)
out <- clust0[order(-clust0[,6]), ]
write.xlsx(out, file = paste0('GM_SEED_cluster_7_',i,'_markers.xlsx'))
...
clust0 <- FindMarkers(merged1,ident.1 = 0,  logfc.threshold = .25)
clust0$ratio <- clust0[,3]/(clust0[,4]+.0000001)
out <- clust0[order(-clust0[,6]), ]
write.xlsx(out, file="MG_WT_harmony_seeded.xlsx", sheetName="cluster 0")
for (i in 1:26) {
clust0 <- FindMarkers(merged1,ident.1 = i,  logfc.threshold = .25)
clust0$ratio <- clust0[,3]/(clust0[,4]+.0000001)
out <- clust0[order(-clust0[,6]), ]
write.xlsx(out, file="MG_WT_harmony_seeded.xlsx", sheetName=paste0("cluster ",i), append=TRUE)
}
...
first <- brewer.pal(n = 8, name = "Set2")
clust0 <- brewer.pal(n = 8, name = "Reds")
clust1 <- brewer.pal(n = 8, name = "Blues")
clust2 <- brewer.pal(n = 8, name = "Greens")
clust3 <- brewer.pal(n = 8, name = "BuPu")
clust4 <- brewer.pal(n = 8, name = "Oranges")
...
clust0$ratio <- clust0[,3]/(clust0[,4]+.0000001)
out <- clust0[order(-clust0[,6]), ]
write.xlsx(out, file = paste0('GM_WT_endo_',i,'_markers.xlsx'))
}
...
DimPlot(c12, group.by=c("total"), cols = c(first[1], first[2],"lightgrey") ,pt.size = 3)
DimPlot(c12, group.by=c("cluster_0"), cols = c(clust0[4],clust0[7],"lightgrey") ,pt.size = 3)
DimPlot(c12, group.by=c("cluster_1"), cols = c(clust1[4], clust1[7], clust1[8],"lightgrey") ,pt.size = 3)
dev.off()
...
clust0$ratio <- clust0[,3]/(clust0[,4]+.0000001)
out <- clust0[order(-clust0[,6]), ]
write.xlsx(out, file = paste0('GM_WT_immune_cell_',i,'_markers.xlsx'))
}
...
Cnn3 <- all %in% WhichCells(object = c12, expression = Cnn3 > 0)
Meis1 <- all %in% WhichCells(object = c12, expression = Meis1 > 0)
total5 <- data.frame(Cnn3 ,Meis1)
total5$ident <- 'zzz'
...
c12[["cluster_4"]] <- total5[,3]
#####
pdf("figure_6a-h.pdf")
DimPlot(c12, group.by=c("seurat_clusters"), cols = colors.use ,pt.size = 3)
DimPlot(c12, group.by=c("c1qb"), cols = c(first[1],"lightgrey") ,pt.size = 3)
DimPlot(c12, group.by=c("tumor.model"),pt.size = 3)
ggbarplot(df[-c(17:20),], x = "cluster", y = "percent", 
          ...
          position = position_dodge(0.8)) + stat_compare_means(aes(group = model), label = "p.signif", hide.ns = T)
DimPlot(c12, group.by=c("cluster_0"), cols = c(clust0[4],clust0[7],"lightgrey") ,pt.size = 3)
DimPlot(c12, group.by=c("cluster_1"), cols = c(clust1[7], clust1[4], clust1[8],"lightgrey") ,pt.size = 3)
DimPlot(c12, group.by=c("cluster_2"), cols = c(clust2[4], clust2[5], clust2[7],clust2[8],"lightgrey") ,pt.size = 3)
DimPlot(c12, group.by=c("cluster_3"), cols = c(clust3[8], clust3[5],"lightgrey") ,pt.size = 3)
DimPlot(c12, group.by=c("cluster_4"), cols = c(clust4[5], clust4[8],"lightgrey") ,pt.size = 3)
dev.off()
...
clust0$ratio <- clust0[,3]/(clust0[,4]+.0000001)
out <- clust0[order(-clust0[,6]), ]
write.xlsx(out, file = paste0('GM_WT_fibroblast_',i,'_markers.xlsx'))
}
...
ggbarplot(df, x = "cluster", y = "percent", 
          ...
          position = position_dodge(0.8)) + stat_compare_means(aes(group = model), label = "p.signif", hide.ns = T)
DimPlot(c12, group.by=c("cluster_0"), cols = c(clust0[4],clust0[7],"lightgrey") ,pt.size = 3)
DimPlot(c12, group.by=c("cluster_1"), cols = c(clust1[4],clust1[7],"lightgrey") ,pt.size = 3)
DimPlot(c12, group.by=c("cluster_2"), cols = c( clust2[5],"lightgrey") ,pt.size = 3)
DimPlot(c12, group.by=c("extra"), cols = c(first[1], first[2],first[3],"lightgrey") ,pt.size = 3)
dev.off()
```
